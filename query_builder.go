...
type ExprVal map[string]interface{}
...
func (q *Query) AddUpdates(attributes []Attribute, action string) {
	var au string
	au1 := msi{}
	Emint := make(map[string]interface{})
	map3 := make(map[string]map[string]map[string]string)
	...
	for _, a := range attributes {
		...
		makeUpdateExpression(Emint, a)

		fmt.Println("lklklklklkllakalkalka", Emint)

		au = a.Name + "=:" + a.ExprVal + "," + au

		for k, _ := range a.MapVal {

			map5[a.MapVal[k].Type] = a.MapVal[k].Value
			map4[k] = map5
			map5 = make(map[string]string)
		}
		...
		au1[":"+a.ExprVal] = map3
		// msi{
		// 	a.Type: map[bool]interface{}{true: a.MapVal, false: a.MapVal}[a.SetType()],
		// }

		//au11 = append(au11, au1)
		//au1 = msi{}
	...
	q.buffer["UpdateExpression"] = "SET " + au[0:len(au)-1]
	q.buffer["ExpressionAttributeValues"] = au1
}

func makeUpdateExpression(Intr interface{}, Atr Attribute) interface{} {

	//mapVal := make(map[string]string)

	fmt.Println("type", reflect.TypeOf(Intr))
	Vbac := make(map[string]map[string]interface{})
	if Atr.Type == TYPE_MAP {
		if Vbac, ok := Intr.(map[string]map[string]interface{}); ok {
			Vbac["M"] = make(map[string]interface{})
		}
		for K, _ := range Atr.MapVal {
			if v, ok := Intr.(map[string]interface{}); ok {
				v[K] = make(map[string]interface{})
				if Vk, ok := v[K].(map[string]interface{}); ok {
					Vbac["M"] = make(map[string]interface{})
					Vbac["M"] = Vk
				}
				makeUpdateExpression(Vbac["M"], Atr.MapVal[K])
				fmt.Println("Val after", v[K], K)
			}
		}
	} else {
		if v, ok := Intr.(map[string]interface{}); ok {
			v[Atr.Type] = Atr.Value
		}

		// mapVal[Atr.Type] = Atr.Value
		// fmt.Println("else", mapVal)
		// Intr = mapVal
		return Intr
		// 	ExMap := Expr["M"]
		// 	for K, _ := range Atr.MapVal {
		// 		makeUpdateExpression(ExMap[K], Atr.MapVal[K])
		// 	}
		// } else {

		// 	if Atr.Name == "" {
		// 		Expr[Atr.Type] = Atr.Name

		// 	} else {
		// 		FinlVal1[Atr.Name] = mapVal
		// 	}
		// }
